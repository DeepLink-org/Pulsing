...
class VllmWorker(Actor):
        ...
        if self._is_ready and self._engine:
            ...
            try:
                ...
                meta.update({
                    ...
                    "max_num_batched_tokens": str(config.scheduler_config.max_num_batched_tokens),
    ...
    async def receive(self, msg: Message) -> Union[Message, StreamMessage]:
        ...
        try:
            if msg.msg_type in ("GenerateRequest", "ChatCompletionRequest"):
            ...
            elif msg.msg_type in ("GenerateStreamRequest", "ChatCompletionStreamRequest"):
                ...
                return Message.from_json("Error", {"error": f"Unsupported type: {msg.msg_type}"})
        except Exception as e:
            logger.exception(f"Error handling {msg.msg_type}: {e}")
            return Message.from_json("Error", {"error": str(e)})

    async def _build_prompt(self, data: Dict[str, Any]) -> Union[TokensPrompt, TextPrompt]:
    ...
    def _build_sampling_params(self, data: Dict[str, Any]) -> SamplingParams:
        ...
        sampling_dict = {
            ...
            "stop": data.get("stop"),
            "max_tokens": data.get("max_new_tokens", data.get("max_tokens", self.default_max_new_tokens)),
        }
        ...
        if self.role == "prefill":
            ...
            sampling_params.extra_args["kv_transfer_params"] = {"do_remote_decode": True}
            
        elif self.role == "decode":
            # Decode 角色：从 prefill_result 中提取 KV 传输参数
            prefill_result = data.get("prefill_result")
            if prefill_result:
                kv_params = prefill_result.get("disaggregated_params", {}).get("kv_transfer_params")
                if kv_params:
                    if sampling_params.extra_args is None:
                        sampling_params.extra_args = {}
                ...
                async for res in results_generator:
                    if res.outputs:
                        ...
                        if self.role == "prefill" and hasattr(res, "kv_transfer_params"):
                            chunk["disaggregated_params"] = {"kv_transfer_params": res.kv_transfer_params}
