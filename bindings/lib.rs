...
fn benchmark_main<'py>(py: Python<'py>, config: PyObject) -> PyResult<Bound<'py, PyAny>> {
    ...
    fn get_opt<T: for<'a> pyo3::FromPyObject<'a>>(dict: &Bound<'_, PyDict>, key: &str) -> PyResult<Option<T>> {
        match dict.get_item(key)? {
    ...
    let max_vus = get_opt::<u64>(&config_dict, "max_vus")?
        .unwrap_or(128);
    
    let duration_str = get_opt_str(&config_dict, "duration")?
    ...
    let num_rates = get_opt::<u64>(&config_dict, "num_rates")?
        .unwrap_or(10);
    
    let profile = get_opt_str(&config_dict, "profile")?;
    
    let benchmark_kind = get_opt_str(&config_dict, "benchmark_kind")?
        .unwrap_or_else(|| "sweep".to_string());
    
    let warmup_str = get_opt_str(&config_dict, "warmup")?
    ...
    let extra_meta = if let Some(s) = extra_meta_str {
        Some(pulsing_bench::parse_key_val(&s)
    ...
    pyo3_async_runtimes::tokio::future_into_py(py, async move {
        pulsing_bench::benchmark_main_async(args).await
