...
PERSONAS = ["性能工程师", "产品经理", "系统架构师", "安全专家", "可靠性运维",
            "研究员", "数据工程师", "成本控制", "用户体验", "测试工程师"]
...
class JudgeActor:
    ...
    async def submit(self, idea: dict, iterations: int) -> dict:
        ...
        print(f"  ✓ [{idea['agent']}] v{idea.get('version',1)} "
    ...
    async def _decide(self):
        ...
        for i, s in enumerate(self.submissions, 1):
            idea = s["idea"]
            print(f"  {i}. [{idea['agent']}] {idea['title']} - {idea.get('score',0)}/10")
...
@remote
class IdeaAgent:
    def __init__(self, persona: str, question: str, judge: str,
                 peers: list[str], max_rounds: int, target_score: int,
                 mock: bool, collab: bool):
    ...
    def assist(self, from_agent: str, context: dict) -> dict:
        ...
        # 基于专业领域给建议
        suggestion = next((s for k, s in MOCK_SUGGESTIONS.items() if k in self.persona),
                          "建议多角度评估")
        if self.idea:
    ...
    async def _generate(self) -> dict:
        if self.mock:
            ...
            idea = {"agent": self.persona, "title": title, "proposal": f"{self.persona}: {title}",
                    "experiments": exps, "risks": risks, "version": 1, "score": 0}
        else:
            llm = await get_llm()
            prompt = f"你是{self.persona}，针对问题提出方案：{self.question}\n输出JSON：{{\"title\":\"\",\"proposal\":\"\",\"experiments\":[],\"risks\":[]}}"
            resp = await llm.ainvoke(prompt)
            data = parse_json(resp.content, {})
            idea = {"agent": self.persona, "title": data.get("title", "方案"),
                    "proposal": data.get("proposal", ""), "experiments": data.get("experiments", []),
                    "risks": data.get("risks", []), "version": 1, "score": 0}
    ...
    async def _critique(self) -> dict:
        ...
        if self.mock:
            ...
            score = min(10, max(1, 5 + len(self.idea.get("experiments", [])) + self.idea.get("version", 1) - 1))
            issues = ["实验不足"] if len(self.idea.get("experiments", [])) < 3 else ["方案可行"]
            consult = [p for p in ["性能工程师", "安全专家"] if p != self.persona][:2]
            critique = {"score": score, "issues": issues, "improvements": ["增加实验"], "consult": consult}
        else:
            llm = await get_llm()
...
{{"score": 1-10分, "issues": ["问题1"], "improvements": ["改进建议1"], "consult": ["建议咨询的专家名称"]}}"""
            resp = await llm.ainvoke(prompt)
            data = parse_json(resp.content, {})
            critique = {"score": data.get("score", 5), "issues": data.get("issues", []),
                        "improvements": data.get("improvements", []), "consult": data.get("consult", [])}
        
        consult_info = f" → 建议咨询: {critique['consult']}" if critique.get('consult') else ""
        print(f"  [{self.persona}] 📋 评审: {critique['score']}/10 | {', '.join(str(i) for i in critique['issues'][:2])}{consult_info}")
        return critique
    ...
    async def _refine(self, critique: dict, advice: list[dict]) -> dict:
        if self.mock:
            ...
            exps.append(f"新增: {critique['improvements'][0][:10]}..." if critique.get("improvements") else "改进实验")
            for a in advice:
                exps.append(f"协作: {a.get('suggestion', '')[:15]}...")
            refined = {**self.idea, "title": self.idea["title"] + " (改进)",
                       "experiments": exps[:5], "version": self.idea.get("version", 1) + 1, "score": 0}
        else:
            llm = await get_llm()
            collab_text = "\n".join([f"- {a['from']}: {a['suggestion']}" for a in advice]) if advice else ""
            prompt = f"你是{self.persona}，改进方案：{json.dumps(self.idea, ensure_ascii=False)}\n问题：{critique['issues']}\n建议：{critique['improvements']}\n{collab_text}\n输出JSON"
            resp = await llm.ainvoke(prompt)
            data = parse_json(resp.content, {})
            refined = {"agent": self.persona, "title": data.get("title", self.idea["title"]),
                       "proposal": data.get("proposal", self.idea["proposal"]),
                       "experiments": data.get("experiments", []), "risks": data.get("risks", []),
                       "version": self.idea.get("version", 1) + 1, "score": 0}
        ...
        for expert in experts:
            ...
            try:
                ...
                if resp.get("suggestion"):
                    advice.append(resp)
                    self.collabs += 1
                    print(f"  [{self.persona}] ✓ 收到 [{resp['from']}]: {resp['suggestion'][:30]}...")
...
async def run(question: str, n: int = 3, rounds: int = 3, target: int = 8,
              timeout: float = 30, mock: bool = False, collab: bool = True):
    print("=" * 50)
    print(f"问题: {question}")
    print(f"Agent: {n} | 轮数: {rounds} | 目标: {target} | 超时: {timeout}s | 协作: {collab}")
    print("=" * 50)
    ...
    async with runtime():
        ...
        for i, persona in enumerate(PERSONAS):
            ...
            agent = await IdeaAgent.spawn(
                persona=persona, question=question, judge="judge",
                peers=peers, max_rounds=rounds, target_score=target,
                mock=mock, collab=collab, name=name
            )
            agents.append(agent)
        ...
        # 等待结果
        results = await asyncio.gather(*[a.get_result() for a in active], return_exceptions=True)
        ...
        for r in results:
            if isinstance(r, dict):
                ...
                print(f"  {s} [{r['persona']}] 分数:{r.get('score',0)} 迭代:{r.get('iterations')} 协作:{r.get('collabs',0)}")
...
if __name__ == "__main__":
    ...
    asyncio.run(run(
        question=args.question, n=args.n_ideas, rounds=args.max_rounds,
        target=args.target_score, timeout=args.timeout,
        mock=args.mock, collab=not args.no_collab
    ))
