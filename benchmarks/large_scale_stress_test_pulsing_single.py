...
class StressTestClient:
    ...
    async def send_single_request(self) -> bool:
        ...
        try:
            if worker_type == "echo":
                msg = Message.from_json("Echo", {"text": f"echo_{random.randint(1, 1000)}"})
                ...
                if random.random() < 0.5:
                    msg = Message.from_json("SetState", {
                        "key": f"key_{random.randint(1, 100)}",
                        "value": random.randint(1, 1000)
                    })
                else:
                    msg = Message.from_json("GetState", {"key": f"key_{random.randint(1, 100)}"})
...
async def main():
    ...
    parser.add_argument("--num-workers", type=int, default=50, help="每种类型的Worker数量")
    parser.add_argument("--port", type=int, default=8000, help="Pulsing端口")
    parser.add_argument("--log-dir", type=str, default="stress_test_logs", help="日志目录")
    ...
    print(f"\n{'='*60}")
    print(f"Pulsing Stress Test (Single Process Mode)")
    ...
    # 打印最终统计
    print(f"\n{'='*60}")
    print(f"Final Statistics (Pulsing Single Process)")
    print(f"{'='*60}")
