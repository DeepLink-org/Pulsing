        ...
        try:
            if worker_type == "echo":
                ...
                if random.random() < 0.5:
                    ...
                    result = await worker.get_state.remote(f"key_{random.randint(1, 100)}")
...
async def main():
    ...
    parser.add_argument("--log-dir", type=str, default="stress_test_logs_ray", help="日志目录")
    ...
    print(f"\n{'='*60}")
    print(f"Ray Stress Test (Single Process Mode)")
    ...
    # 打印最终统计
    print(f"\n{'='*60}")
    print(f"Final Statistics (Ray Single Process)")
    print(f"{'='*60}")
