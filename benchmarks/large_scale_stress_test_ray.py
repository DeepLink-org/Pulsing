...
class StressTestClient:
        ...
        try:
            ...
            if base_type == "echo":
                ...
                if msg_type == "SetState":
                    ...
                    result = await worker_ref.set_state.remote(payload["key"], payload["value"])
                else:
    ...
    async def check_cluster_health(self) -> tuple[bool, str]:
        ...
        try:
            ...
            if num_nodes == 0:
                # 尝试其他方式获取节点数
                num_nodes = len([k for k in cluster_resources.keys() if k.startswith("node:")])
...
async def main():
    ...
    if not ray.is_initialized():
        if args.address:
            ...
            print(f"[Process {rank}] Ray initialized: {num_cpus} CPU, "
