...
class StressTestStats:
    ...
    def add_request(self, success: bool, latency_ms: float, error: Optional[str] = None):
    ...
    def add_stream(self, success: bool, latency_ms: float, error: Optional[str] = None):
...
class StressTestClient:
    ...
    def __init__(
        ...
        self.expected_worker_types = expected_worker_types or ["echo", "compute", "stream", "batch", "stateful"]
    ...
    def _extract_base_worker_type(self, worker_type: str) -> str:
        ...
        if len(parts) > 0 and parts[0] in ["echo", "compute", "stream", "batch", "stateful"]:
            return parts[0]
    ...
    async def check_cluster_health(self) -> tuple[bool, str]:
        ...
        try:
            ...
            if len(alive_members) < self.expected_nodes:
                return False, f"Only {len(alive_members)}/{self.expected_nodes} nodes alive"
            ...
            for member in alive_members:
                ...
                for worker_type in self.expected_worker_types:
                    try:
                        # 尝试查找该节点上的worker
                        instances = await self.system.get_named_instances(f"{worker_type}_0")
                ...
                # 检查是否缺少worker类型
                missing = [wt for wt in self.expected_worker_types if wt not in node_workers]
                if missing:
            ...
            return True, f"All {len(alive_members)} nodes healthy with {len(self.expected_worker_types)} worker types each"
    ...
    async def run_stress_test(self, duration: float):
        ...
        print(f"[StressTest] Expected: {self.expected_nodes} nodes, {len(self.expected_worker_types)} worker types per node")
        
        # 初始健康检查
        is_healthy, health_msg = await self.check_cluster_health()
        if not is_healthy:
            print(f"[StressTest] ⚠️  WARNING: Initial health check failed: {health_msg}")
            print(f"[StressTest] Continuing anyway, but monitoring cluster health...")
        else:
        ...
        async def worker_loop():
            ...
            while self.running and time.time() < end_time:
                ...
                is_remote = False
                if remote_workers and (not local_workers or random.random() < use_remote_probability):
        ...
        async def report_loop():
            ...
            while self.running and time.time() < end_time:
                ...
                if total_worker_requests > 0:
                    ...
                    print(f"  Worker Selection: {remote_pct:.1f}% remote, {local_pct:.1f}% local")
...
async def main():
    ...
    print(f"[Process {rank}] Waiting for cluster to stabilize ({stabilize_timeout}s)...")
    ...
    if len(members) < world_size:
        ...
        print(f"    1. Some processes haven't started yet")
        print(f"    2. Network partition (split-brain)")
        print(f"    3. Processes crashed or exited")
    ...
    if missing_workers:
        ...
        print(f"  This node does not have all required worker types!")
        print(f"  Expected: {required_workers}, Got: {spawned_workers}")
    else:
        print(f"  ✓ All {len(required_workers)} worker types spawned successfully on node {rank}")
    
    # 等待所有进程启动worker并完成注册
    # 在大规模场景下，Gossip传播需要更多时间
    stabilize_timeout = args.stabilize_timeout
    print(f"[Process {rank}] Waiting for all workers to register ({stabilize_timeout}s)...")
    await asyncio.sleep(stabilize_timeout)
    ...
    if resolved_count < total_expected:
        print(f"  ⚠️  Warning: Only resolved {resolved_count}/{total_expected} remote workers")
        print(f"  This may indicate cluster partition or nodes not fully registered")
