...
"""Explicit legacy Python-owned Forge loop.

This module is not used by the default ``ForgeAgent``. It remains temporarily
available for development while Python-only tools move behind worker adapters.
"""
...
@dataclass
class LegacyPythonForgeAgent:
    """Thin Host: LLM loop + Forge tools + default CLI event output.

    Example::

        agent = LegacyPythonForgeAgent(cwd=".", provider="demo")
...
__all__ = ["LegacyPythonForgeAgent"]
