...
async def query_cluster(seeds: list[str], all_actors: bool, output_format: str):
    ...
    if len(real_members) > MAX_NODES_DISPLAY:
        print(f"Warning: Cluster has {len(real_members)} nodes, showing first {MAX_NODES_DISPLAY}")
    ...
    # Summary
    if output_format == "table":
        total = sum(len(d.get("actors", [])) for d in all_nodes_data) if all_nodes_data else 0
        print(f"{'='*70}")
...
async def _get_node_actors_via_instances(system, target_node_id: str, all_actors: bool) -> list[dict]:
