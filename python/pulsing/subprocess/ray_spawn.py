...
def _replace_addr_with_routable_ip(addr: str) -> str:
    ...
    # If host is already a specific IP (not 0.0.0.0 or localhost), keep it
...
def _build_pulsing_node_actor_cls():
    ...
    class PulsingNodeActor:
        ...
        async def start(self) -> dict:
            ...
            # Spawn ProcessActor locally on this node
...
async def ray_spawn_process_actor(
    ...
    **process_kwargs,
    ...
    proxy = await ProcessActor.resolve(actor_name, node_id=info["node_id"], timeout=30)

    # Store Ray Actor handle on proxy to prevent GC
    proxy._ray_node_actor = ray_actor
