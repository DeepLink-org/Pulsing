...
try:
    ...
    raise ImportError(
        "pulsing.ray requires Ray. Install with: pip install 'ray[default]'"
    )
...
async def _do_init(addr, seeds=None):
    from pulsing.actor import init

    return await init(addr=addr, seeds=seeds)


async def _do_shutdown():
    from pulsing.actor import shutdown
...
__all__ = ["init_in_ray", "async_init_in_ray", "cleanup"]
